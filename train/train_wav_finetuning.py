# train/train_wav_finetuning.py

import torch
import time
from config.config import get_config, activation_dict
from data_loaders.loader import get_loader_wav
from solvers.solver_wav_finetuning import Solver_fine

def main():
    # 1. 설정 객체 생성 (명령행 인자 또는 기본값 사용)
    train_config = get_config(mode='train')
    test_config  = get_config(mode='test')

    # 2. device 설정: config에 device 속성이 없다면 기본값 지정
    ...
    if not hasattr(train_config, 'activation'):
        # activation_dict는 config/config.py에 정의되어 있음 (예: {'relu': torch.nn.ReLU, ...})
    ...
    if not hasattr(train_config, 'name'):
        ...
        train_config.name = "train_wav_finetune_" + time.strftime("%Y%m%d_%H%M%S", time.localtime())
    ...
    # 5. WAV 파인튜닝 데이터 로더 생성 (finetuning=True)
    train_loader = get_loader_wav(train_config, finetuning=True, shuffle=True)
    test_loader  = get_loader_wav(test_config, finetuning=True, shuffle=False)

    # 6. Solver_fine 객체 생성 및 학습 수행
    solver = Solver_fine(train_config, test_config, train_loader, test_loader, is_train=True)
    solver.build()
    solver.train()
